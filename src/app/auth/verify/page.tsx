...
// ---------------------------------------------------------------------------
// Background decoration (left panel) — 100% identical to login, register & forgot-password
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Animation variants — exact match to all other auth pages
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 6-digit OTP Input Component (auto-focus, paste support, numeric only)
// ---------------------------------------------------------------------------
...
function OTPInput({
  ...
  const handleInput = (index: number, input: string) => {
    if (!/^\d*$/.test(input)) return; // numeric only

    const newValue = [...value];
    newValue[index] = input.slice(-1); // take only last character
    onChange(newValue);
    ...
    // Auto-process when last digit is entered
  ...
  const handleKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Backspace") {
      ...
      // Clear current field (handled by onChange)
    }
  };
    ...
    <div className="flex items-center justify-center gap-3">
      {value.map((digit, index) => (
        <input
          ...
          onPaste={handlePaste}
          className="w-12 h-14 text-center text-4xl font-semibold bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.09] rounded-3xl focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all text-slate-900 dark:text-white"
...
export default function VerifyPage() {
  ...
  const { verifyEmail, resendVerificationEmail, isLoading } = useAuthStore(); // ← Adjust method names if your store uses different ones
  ...
  // Auto-process when all 6 digits are filled (triggered from OTPInput)
  const handleVerify = (code: string) => {
    ...
    startTransition(async () => {
      try {
        await verifyEmail(code); // ← Your store method (e.g. verifyOtp, confirmEmail, etc.)
        setIsSuccess(true);
        toast.success("Email verified successfully!");
        // Optional: redirect immediately after success animation
        setTimeout(() => {
  ...
  const handleResend = () => {
    ...
    startTransition(async () => {
      try {
        await resendVerificationEmail(); // ← Your store method
        setResendTimer(60);
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      {/* ── LEFT: brand panel — identical to all other auth pages */}
      ...
      {/* ── RIGHT: form panel — exact styling match */}
      <div className="flex-1 flex flex-col items-center justify-center min-h-screen px-5 py-12 sm:px-10 bg-white dark:bg-[#0d1117]">
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {!isSuccess ? (
              ...
              >
                ...
                {/* Server error banner (styled exactly like login/register/forgot) */}
                <AnimatePresence>
                  {serverError && (
                    <motion.div
                ...
                <motion.div variants={fadeUp} className="text-center">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-indigo-600 disabled:text-slate-300 transition-colors"
                  >
                    {resendTimer > 0 ? (
                      <>
              ...
              >
                ...
                <p className="text-emerald-600/80 dark:text-emerald-400/70 text-[15px]">
                  You’re all set. Welcome to Accountant Pathfinder.
                </p>
                <button
                  onClick={() => router.push("/dashboard")}
                  className="mt-8 w-full h-11 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl transition-all"
                >
          ...
          {/* Trust signals (only when not success) */}
          {!isSuccess && (
            <motion.div
              variants={fadeUp}
